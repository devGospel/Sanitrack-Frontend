...
import EditCertificationModal from "./modals/EditCertifications";


const CertificationTable = () => {
  const { getAllCertifications, loading, allCertifications } = useReport();
  ...
  const currentItems = allCertifications?.slice(indexOfFirstItem, indexOfLastItem);
  ...
  const nextPage = () => {
    if (currentPage !== Math.ceil(allCertifications.length / itemsPerPage)) {
      setCurrentPage(currentPage + 1);
    }
  };
  const firstPage = () => {
  ...
  const lastPage = () => {
    setCurrentPage(Math.ceil(allCertifications?.length / itemsPerPage));
  };
  ...
  return (
    <>
      ...
      <div className="w-full overflow-auto no-scrollbar  h-auto">
        ...
        {allCertifications?.length === 0 && !loading && (
      ...
      <FreePagination
        ...
        totalItems={allCertifications?.length}
        paginate={paginate}
      ...
      >
        <EditCertificationModal closeModal={closeModalAdd} />
      </ModalComponent>
    </>
  );
};
