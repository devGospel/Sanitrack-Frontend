...
const FacilityTable = ({}) => {
  const { getAllFacilities, loading, allFacilities, getManagerFacilities,getFacilitiesForManager } =
    useFacilities();
  ...
  useEffect(() => {
    ...
    if (role?.toLowerCase() == "manager") {
      ...
      console.log('is manager duh')
    } else getAllFacilities();
  }, []);
  const dispatch = useDispatch();
  const [currentPage, setCurrentPage] = useState(1);
  const [itemsPerPage] = useState(10);
  
  console.log("ghgh",allFacilities)

  // ...

  const indexOfLastItem = currentPage * itemsPerPage;
  const indexOfFirstItem = indexOfLastItem - itemsPerPage;
  const currentItems = allFacilities?.slice(indexOfFirstItem, indexOfLastItem);
  ...
  const nextPage = () => {
    if (currentPage !== Math.ceil(allFacilities.length / itemsPerPage)) {
      setCurrentPage(currentPage + 1);
    }
  };
  const firstPage = () => {
  ...
  const lastPage = () => {
    setCurrentPage(Math.ceil(allFacilities?.length / itemsPerPage));
  };
  ...
  return (
    <>
      <div className="w-full overflow-auto no-scrollbar  h-auto">
                ...
                <tr key={item?._id} className="border-b border-gray-200">
                  ...
                  <td className="px-5 py-3   text-sm">
                    <p
                      className={` whitespace-no-wrap text-sm font-normal    capitalize`}
                    >
                      {`${item?.facilityPrefix}${item?.facilityCode}`}
                    </p>
                  </td>
                  <td className="px-5 py-3   text-sm">
                    <p
                  ...
                  <td className="px-5 py-3   text-sm">
                    <p
                      className={` whitespace-no-wrap text-sm font-normal    capitalize`}
                    >
                      {item?.roomCount ? item?.roomCount : 0}
                    </p>
                  </td>
                  <td className="px-5 py-3    text-sm flex items-center">
                    {newRole?.toLowerCase() === "admin" && (
                      <button
                        ...
                        className={` whitespace-no-wrap text-xs font-thin dark:bg-white bg-sanBlue dark:text-black ml-3 p-2 rounded-md text-white cursor-pointer  `}
                      >
                        Assign
                      </button>
                    )}
                    <>
                      <Link
                        ...
                        href={`/dashboard/facilities/${item?._id}`}
                        onClick={() => {
                          window?.localStorage.setItem(
                            "singleItem",
                            JSON.stringify(item)
        ...
        {allFacilities?.length === 0 && !loading && (
      ...
      <FreePagination
        ...
        totalItems={allFacilities?.length}
        paginate={paginate}
