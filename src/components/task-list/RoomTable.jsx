...
export default function AssetTable({roomData}) {
    ...
    const totalPages =
        roomData.length > 0 ? Math.ceil(roomData.length / itemsPerPage) : 0;
    const currentItems = roomData?.slice(indexOfFirstItem, indexOfLastItem);
    ...
    const nextPage = () => {
        if (currentPage !== Math.ceil(roomData.length / itemsPerPage)) {
            setCurrentPage(currentPage + 1);
        }
    };
    const firstPage = () => {
    ...
    const lastPage = () => {
        setCurrentPage(Math.ceil(roomData?.length / itemsPerPage));
    };
            ...
            {isModalOpenDel && (
                ...
                >
                    <div className='p-4 flex flex-col'>
                        ...
                        <h1 className='text-sm py-2'>Are You sure u want to delete this room {currentUserId} ? This action cannot be undone</h1>
