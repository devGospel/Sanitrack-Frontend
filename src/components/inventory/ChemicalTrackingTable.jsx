...
import RemoveTraining from "../lms/trainings/DeleteTrainingModal";
import EditInventoryModal from "./modals/EditInventoryModal";
import RemoveInventory from "./modals/RemoveInventory";
import useInventory from "@/hooks/useInventory";

const ChemicalTrackingTable = () => {
  const { getChemicals, allChemicals,loading } = useInventory();
  useEffect(() => {
    getChemicals();
  }, []);
  ...
  const currentItems = allChemicals?.slice(indexOfFirstItem, indexOfLastItem);
  ...
  const nextPage = () => {
    if (currentPage !== Math.ceil(allChemicals.length / itemsPerPage)) {
      setCurrentPage(currentPage + 1);
    }
  };
  const firstPage = () => {
  ...
  const lastPage = () => {
    setCurrentPage(Math.ceil(allChemicals?.length / itemsPerPage));
  };
  ...
  return (
    <>
      <div className="w-full overflow-auto no-scrollbar  h-auto">
        ...
        >
          <EditInventoryModal closeModal={closeModalEdit} />
        </ModalComponent>
        <ModalComponent
        ...
        >
          <RemoveInventory closeModal={closeModalDel} />
        </ModalComponent>
      ...
      <FreePagination
        ...
        totalItems={allChemicals?.length}
        paginate={paginate}
...
export default ChemicalTrackingTable;
