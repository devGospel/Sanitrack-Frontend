...
import TaskListHeader from "./TaskManageHeader";
import { useDispatch } from "react-redux";
import TaskManagementHeader from "./TaskManageHeader";
    ...
    <table className="table-auto w-full leading-normal no-scrollbar overflow-scroll lg:overflow-hidden">
      <thead>
        <tr className="border-gray-200  whitespace-nowrap border bg-white dark:bg-black dark:border-gray-50">
          ...
          <th className="px-5 py-3 text-right text-sm font-semibold text-gray-500 dark:text-white capitalize tracking-wider">
            Actions
          </th>
        </tr>
      </thead>
      <tbody className="[&>*:nth-child(odd)]:bg-sanLightBlue dark:[&>*:nth-child(odd)]:bg-slate-900 dark:text-white text-black [&>*:nth-child(even)]:text-black  [&>*:nth-child(even)]:bg-white dark:[&>*:nth-child(even)]:bg-sanBlue dark:[&>*:nth-child(even)]:text-white shadow-lg w-full">
        {data?.map((table_data) => (
          ...
          >
            ...
            <td className="px-5 py-2   text-sm">
              <div className="flex gap-2 items-center">
                <p>{table_data?.isDefault ? "Default" : ""}</p>
              </div>
            </td>
...
export default function TaskTable({ data, loading }) {
  ...
  return (
    <div className="w-full bg-white dark:bg-black h-screen mx-auto">
      <div className=" w-full flex flex-col no-scrollbar px-4 sm:px-6 py-10">
        <TaskManagementHeader
          heading={"Task Management"}
          paragraph={
            "Manage, create and oversee task  management within the system."
          }
        ...
        {!loading && data?.length < 1 && (
          <div className="flex  justify-center  w-full pt-5">
            <p className="text-red-500 text-lg font-bold">No Tasks available</p>
          </div>
        )}
        <FreePagination
        ...
        >
          <div className="p-4 flex flex-col">
            ...
            <h1 className="text-sm py-2">
              Are You sure u want to delete this Task {currentUserId} ? This
