...
const OverviewCards = () => {
  return (
    <div className="grid lg:grid-cols-3 grid-cols-2  gap-6">
      {data?.map((card, i) => {
        const { iconBg, icon, countColor, count, label, small } = card;
        return (
          <Link
            href={card?.link}
            className={` w-full  items-center gap-2 p-2 bg-white dark:bg-black hover:bg-white shadow-[5px_5px_0px_0px_#DADFFF] dark:shadow-[5px_5px_0px_0px_#5B5BE3] rounded-[30px] ${
              small ? "flex flex-col" : "hidden lg:flex flex-col"
            }`}
          >
