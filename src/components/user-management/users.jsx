...
const DataTable = ({ title, data, openModalEdit, openModalDel, isManager }) => (
  <div className="overflow-x-auto w-full  flex flex-col no-scrollbar px-4 py- h-auto">
    <h1 className="font-bold  text-lg">{title}</h1>
    <table className="table-auto w-full leading-normal mt-4 no-scrollbar overflow-scroll lg:overflow-hidden">
      <thead>
          ...
          >
            <td className="px-5 py-2   text-sm">
              <div className="flex gap-2 items-center">
                ...
                <p>{isManager ? table_data.user_name : table_data.username}</p>
              </div>
            </td>
            <td className="px-5 py-2 text-sm">{table_data.email ?? "N/A"}</td>
            <td className="px-5 py-2 text-sm">
              {table_data.phone_number ?? "N/A"}
            </td>
            <td className="px-5 py-2 text-sm flex items-center">
              <>
                {!isManager && (
                  <Link
                    href={`/dashboard/user-management/edit-users/${table_data._id}`}
                  >
                    <button
                      // onClick={() => openModalEdit(table_data._id)}
                ...
                <Link
                  ...
                  onClick={() =>
                    window?.localStorage.setItem(
                      "username",
                      JSON.stringify(isManager ? table_data.user_name : table_data.username)
                    )
                  }
...
export default function UserManagement() {
  ...
  const managerData = allFacilityCleaners.concat(allFacilityInspectors);
  console.log("first,", managerData);
  const isManager = role?.toLocaleLowerCase() === "manager";
  ...
  const { getAllUsers, allUsers, editUser, loading } = useUser();
  const data = isManager ? managerData : allUsers;
  ...
  //   e.preventDefault();
  //   try {
  //     await editUser({ ...formData, _id: currentUserId });
  //     toast.success('User edited successfully');
  //     closeModalEdit(); // Close the modal after successful edit
  //   } catch (error) {
  ...
  return (
    <div className=" h-screen  w-full mx-auto bg-white dark:bg-slate-800">
      <div className=" w-full flex flex-col no-scrollbar px-4 sm:px-6 py-10">
        <TopLayout
          heading={"User Management"}
          paragraph={
            "Addminister and oversee user account and privilages within the platform"
          }
          isManager={isManager}
          title="Add users"
          path={"/dashboard/user-management/add-users"}
        />
          ...
          <div className='p-4 flex flex-col'>
            <form className=' p-4' onSubmit={handleSubmit}>
              ...
              <input
                ...
                className='p-2 w-full my-2 rounded-md outline-none border border-gray-300 bg-white text-black font-thin'
              />
              <button className='bg-blue-500 text-center p-3 rounded-md w-full my-2'>Edit</button>
            </form>
          </div>
