...
const EvidenceDetails = () => {
  ...
  const { allWorkOrderEvidence, getWorkOrderEvidence, loading } = useEvidence();

  useEffect(() => {
    getWorkOrderEvidence(params?.id);
  }, []);
  ...
  console.log("first", allWorkOrderEvidence);
  return (
    <div className="bg-white dark:bg-slate-800 text-black  h-screen lg:p-10 p-5 w-full">
      ...
      <>
        {allWorkOrderEvidence?.length > 0 ? (
          <div className="grid grid-cols-2 lg:grid-cols-6 mt-5 gap-4">
            {allWorkOrderEvidence?.map((item) => (
              <Link
                href={`/dashboard/evidence/task/${item?._id}`}
                key={item?._id}
                // onClick={() => {
              ...
              >
                ...
                <p className="font-thin italic text-xs text-center text-black dark:text-white">{`${
                  item?.assetId?.name
                } ${formatDate(item?.scheduledDate)}`}</p>
              </Link>
            ))}
          </div>
        ) : (
